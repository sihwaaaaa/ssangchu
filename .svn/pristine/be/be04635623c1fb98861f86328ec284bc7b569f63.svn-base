		...
		<div class="container h-100">
			<div class="blog-banner">
				<div class="text-center">
					<h1>싱싱한 <span style="color: #78e284">${category} </span> 제품 만나보기</h1>
					<nav aria-label="breadcrumb" class="banner-breadcrumb">
						<ol class="breadcrumb">
							<li class="breadcrumb-item"><a href="${pageContext.request.contextPath}/">Home</a></li>
							<li class="breadcrumb-item active" aria-current="page"><a href="${pageContext.request.contextPath}/board/category/${cateNo}"> ${category}</a></li>
						</ol>
					</nav>
				</div>
			</div>
								...
								<ul class="pagination">
									...
									<c:if test="${page.doublePrev}">
										<li class="page-item"><a class="page-link"
											href="${cateNo}${page.cri.queryString}&pageNum=${page.startPage -1}&option=${param.option}"><i
												class="fas fa-angle-double-left"></i></a></li>
									</c:if>
									<c:if test="${page.prev}">
										<li class="page-item"><a class="page-link"
											href="${cateNo}${page.cri.queryString}&pageNum=${page.cri.pageNum-1}&option=${param.option}"><i
									...
									<c:forEach begin="${page.startPage}" end="${page.endPage}"
										...
										<li class="page-item ${page.cri.pageNum == i ? 'active' : ''}"><a
											class="page-link"
											href="${cateNo}${page.cri.queryString}&pageNum=${i}&option=${param.option}">${i}</a></li>
									</c:forEach>
									...
									<c:if test="${page.doubleNext}">
										<li class="page-item"><a class="page-link"
											href="${cateNo}${page.cri.queryString}&pageNum=${page.endPage + 1}&option=${param.option}"><i
